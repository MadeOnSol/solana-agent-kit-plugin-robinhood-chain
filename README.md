...
> **0.8.1** — version alignment with the wider RHC SDK release: WebSocket stream tokens (`POST /api/v1/stream/token`) **never expire** since 2026-08-27 — the same token comes back on every call, `expires_at` / `next_refresh_at` are always `null`, and only an explicit `{"rotate": true}` replaces one. This plugin's tools are REST-only and mint no stream token, so nothing here changed behavior.
...
> **New in 0.9.0 — a real registration bug fixed.** An internal agentic-infra coverage audit found `tokenTopTraders`, `tokenFlow`, `tokenPeakHistory`, `tokenRisk`, and `tokenHolders` were exported from `tools/index.ts` but never imported into `index.ts` — present in the package, completely unreachable via `plugin.methods`. Fixed, along with adding `tokenEarlyBuyers` (`GET /rhc/tokens/{address}/early-buyers`) — first buyers of a token, ranked, with still-holding status, previously unreachable from any agent surface. 65 methods, was 59.
>
