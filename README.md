...
> **New — 30 tools (was 14).** Deployer intelligence deepened: `deployerTrajectory` (is this deployer improving or declining), `deployerTokens` (the paginated launch list), `deployerHistory`, `deployerStats`, `deployerAlerts` and `deployerBestTokens` (reputable deployers only), plus `recentBonds`. New KOL tools `kolCoordination` and `kolFirstTouches`, and two batch tools — `tokenBatch` (50 tokens) and `tokensBatchBuyerQuality` (20 tokens) — that each count as one request.
